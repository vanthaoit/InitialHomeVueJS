<template>
  <article class="app-body" id="post-markdown" :class="{ 'fade' : loading }">
    <h2>{{ article.Name }}</h2>
    <p class="post-date">{{ article.DateCreated | formatDate}}</p>
    <p v-if="article" class="markdown-content">
    {{article.Description }}
    </p>
    <div class="tags">
      <span>Tags:</span>
      <span class="tag-code">{{article.SeoDescription}}</span>
    </div>
    <!-- <div class="qrcode">
